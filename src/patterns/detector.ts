/**
 * Ghost Evaluator v15.1 - Pattern Detector
 * =========================================
 ...
 * - Same trigger as ZZ: Indicator (≥2) + continuous alternation
 * - Predicts: SAME as current direction (break the alternation)
 *
 * Example: GGRGR → Current is R, AntiZZ predicts R (alternation breaks)
 *          GGRGRG → Current is G, AntiZZ predicts G (alternation breaks)
 *
 * UPDATED v16: Lowered threshold from ≥3 to ≥2 for indicator
 */
function detectAntiZZ(runData: RunData, blockIndex: number): PatternSignal | null {
  // Need at least 4 runs to establish the pattern (indicator + 3 singles)
  if (runData.lengths.length < 2) return null;
  ...
  for (let i = runData.lengths.length - 2; i >= 0; i--) {
    if (runData.lengths[i] >= 2) {
      ...
      if (allOnes) {
        const onesCount = runData.lengths.length - i - 1;
        // Need at least 3 ones for initial trigger
        if (onesCount >= 3) {
          indicatorIndex = i;
          break;
        }
  ...
  // AntiZZ predicts SAME as current direction (alternation breaks)
  const currentDir = runData.currentDirection;
  // Store the indicator direction (the direction of the ≥2 run)
  const indicatorDirection = runData.directions[indicatorIndex];
  ...
  return {
    pattern: 'AntiZZ',
    signalIndex: blockIndex,
    expectedDirection: currentDir, // Same as current
    ts: new Date().toISOString(),
    indicatorDirection, // Save for persistence after break
  };
}
